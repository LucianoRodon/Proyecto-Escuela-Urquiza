...
const ActualizarHorarioPrevio = ({ hpdId, dmId }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`/api/actualizarHPD/${hpdId}/${dmId}`, {
        method: 'PUT',
        body: formData,
        headers: {
      ...
      console.error('Error al actualizar horario previo del docente:', error);
    }
  };
